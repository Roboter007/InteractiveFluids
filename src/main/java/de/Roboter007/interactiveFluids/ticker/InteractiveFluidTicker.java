...
public class InteractiveFluidTicker extends FluidTicker {
            ...
            if (fluidSectionBelow != null && blockSectionBelow != null) {
                ...
                if (isSolid(blockBelow) || fluidBelowId != 0 && fluidBelowId != spreadFluidId && fluidBelowId == fluidId) {
                    ...
                    if (this.getFluidCollisionMap().containsKey(IFOperators.ALL_BLOCKS)) {
                        ...
                        config = this.getFluidCollisionMap().get(fluidBelow.getId());
                    } else {
                        config = null;
                    }
                ...
                if (config != null) {
                    ...
                    if (block != null && block.getDrawType() != DrawType.Cube) {
                        ...
                        if (blockBoundingBoxes != null) {
                            ...
                            executeCollision(world, flowPhase, fluidId, config, otherBlockSection, blockX, dBlockY, blockZ);
                        }

                    } else {
                        executeCollision(world, flowPhase, fluidId, config, otherBlockSection, blockX, blockY, blockZ);
                    }
                }
            }
        }
    ...
    private static void executeCollision(
            @Nonnull World world,
            @Nonnull FlowPhase flowPhase,
            int fluidId,
            @Nonnull BCConfigEntry config,
            BlockSection targetSection,
        ...
        if (resultConfig.revertBlock()) {
            if (flowPhase == FlowPhase.Spread) {
                BlockType resultBlock = resolveResultBlock(resultConfig);
                if (resultBlock == null && resultConfig.getBlockToPlaceIndex() != Integer.MIN_VALUE) {
                    return;
                }
                FluidCollisionManager.addDelayedCollision(
                        world, blockX, blockY, blockZ,
                        expectedType, resultBlock,
                        fluidId,
                ...
                FluidCollisionManager.addDelayedRevert(
                        world, blockX, blockY, blockZ,
                        expectedType,
                        fluidId,
                        resultConfig.getBlockPlaceDelay(),
    ...
    @Nullable
    private static BlockType resolveResultBlock(@Nonnull BCResultConfig resultConfig) {
        int blockToPlaceId = resultConfig.getBlockToPlaceIndex();
        if (blockToPlaceId == Integer.MIN_VALUE) return null;

        BlockType resultBlock = BlockType.getAssetMap().getAsset(blockToPlaceId);
        if (resultBlock == null) return null;
